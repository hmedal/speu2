...
if __name__ == "__main__":
    ...
    probs_file = 'params/fac_pro_probs_and_costs_' + generate_probs_and_scens_files.get_params_string_probs(args.num_alloc_levels,
                                                                                                    args.num_states) + '.json'
    params_file = 'params/params_Daskin_' + generate_probs_and_scens_files.get_params_string_params(
        args.num_facs,
        args.num_alloc_levels,
        args.num_states) + '.json'
    model = speu.create_model_object(True, params_file, scens_file, probs_file, args.debug)
    model.solve()
